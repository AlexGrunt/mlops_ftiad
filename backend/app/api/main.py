...
@app.get("/models/", response_model=List[schemas.ModelBase], status_code=status.HTTP_200_OK)
def get_models():
...
def refit_model(model: Union[schemas.RidgeModel, schemas.LassoModel, schemas.DecisionTreeModel]):
    return crud.refit_model(model)

@app.post("/model/fit/", status_code=status.HTTP_200_OK)
def fit_model(model: Union[schemas.RidgeModel, schemas.LassoModel, schemas.DecisionTreeModel]):
...
@app.get("/models/predict/", status_code=status.HTTP_200_OK)
def predict_model(model: schemas.ModelBase):
    return crud.get_predictions(model)
